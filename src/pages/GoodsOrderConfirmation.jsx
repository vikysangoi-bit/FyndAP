...
const STORAGE_KEY = 'goodsOrderConfirmation_visibleColumns';
const SERVICE_TYPE = 'GaaS';

export default function GoodsOrderConfirmation() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      await base44.functions.invoke('logAuditEntry', {
        ...
        details: 'Created goods order confirmation'
      });
    },
    onSuccess: () => {
    ...
    mutationFn: async ({ id, data }) => {
      ...
      if (user.role === 'admin') {
        ...
        await base44.functions.invoke('logAuditEntry', {
          ...
          entity_id: id,
          entity_name: data.orderFormNo,
          details: 'Updated goods order confirmation'
        });
      } else {
        await base44.entities.ApprovalRequest.create({
    ...
    mutationFn: async (id) => {
      ...
      if (user.role === 'admin') {
        ...
        await base44.functions.invoke('logAuditEntry', {
          ...
          details: 'Deleted goods order confirmation'
        });
      } else {
        await base44.entities.ApprovalRequest.create({
  ...
  const filteredOrders = orders.filter(order => {
    ...
    if (order.serviceName !== SERVICE_TYPE) return false;
    ...
    return matchesSearch && matchesStatus && matchesClient && matchesSalesPerson && matchesDeliveryMonth;
  });
  ...
  const allColumns = [
    ...
    { id: 'customerBrand', header: "Brand", accessor: "customerBrand" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <PageHeader
          title="Order Confirmation for Goods"
          subtitle="Manage goods-type order confirmations (GaaS)"
          onAdd={() => { setEditingOrder(null); setViewingOrder(null); setShowForm(true); }}
          addLabel="New Goods Order"
        >
          <ColumnSelector
          ...
          <SyncDropdown
            ...
            onExportToExcel={() => {
              const headers = ['orderFormNo', 'customerCode', 'customerName', 'customerBrand', 'orderFormValue', 'expectedDelivery', 'status'];
              const rows = filteredOrders.map(o => [o.orderFormNo, o.customerCode, o.customerName, o.customerBrand, o.orderFormValue, o.expectedDelivery, o.status]);
              const csv = [headers, ...rows].map(row => row.map(cell => `"${cell}"`).join(',')).join('\n');
              const blob = new Blob([csv], { type: 'text/csv' });
              const url = window.URL.createObjectURL(blob);
              const a = document.createElement('a');
              a.href = url;
              a.download = `goods_orders_${new Date().toISOString().split('T')[0]}.csv`;
              a.click();
            }}
          />
        ...
        <div className="space-y-4">
          ...
          <FilterBar
            ...
            filters={[
              ...
              },
              {
                ...
                options: [...new Set(filteredOrders.map(o => {
                  if (!o.expectedDelivery) return null;
                  const d = new Date(o.expectedDelivery);
                  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
                }).filter(Boolean))].sort().reverse().map(month => {
                  const [year, m] = month.split('-');
                  return { value: month, label: new Date(year, parseInt(m) - 1).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) };
                })
              },
              {
                key: 'status',
          ...
          {isLoading ? (
            ...
            <EmptyState
              title="No goods order confirmations found"
              description="Create a new order confirmation for goods to get started."
              onAdd={() => { setEditingOrder(null); setViewingOrder(null); setShowForm(true); }}
              addLabel="New Goods Order"
            />
          ) : (
